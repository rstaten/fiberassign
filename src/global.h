...
// Collect -----------------------------------------------------------
// From the HTM tree, collects for each fiber and for each plate, the available
// galaxies
void collect_galaxies_for_all (const MTL & M,
                               const htmTree <struct target> & T, Plates & P,
                               const FP & pp, const Feat & F);

// From the previous computations, computes the inverse map, that is to say,
// for each target, computes the tile-fibers which can reach it
...
void improve (MTL & M, Plates & P, const FP & pp, const Feat & F,
              Assignment & A, int jstart);

void improve_from_kind (MTL & M, const Plates & P, const FP & pp,
                        const Feat & F, Assignment & A, str kind,
                        int next = -1);

void update_plan_from_one_obs (int j, const Gals & G, MTL & M, Plates & P,
                               const FP & pp, const Feat & F, Assignment & A);

// void redistribute_tf(MTL& M, Plates&P, const FP& pp, const Feat& F,
// Assignment& A, int next=-1);
void redistribute_tf (MTL & M, Plates & P, const FP & pp, const Feat & F,
                      Assignment & A, int jstart);
...
void write_FAtile_ascii (int j, str outdir, const MTL & M, const Plates & P,
                         const FP & pp, const Feat & F, const Assignment & A);

void fa_write (int j, str outdir, const MTL & M, const Plates & P,
               const FP & pp, const Feat & F, const Assignment & A);

void write_save_av_gals (int j, str outdir, const MTL & M, const Plates & P,
                         const FP & pp, const Feat & F);
